...
class ModelTemplate:
    ...
    keys_special = []  # list of strings to handle like ND tensors (save separately)
...
class ModelHyVid(ModelTemplate):
    ...
    def __init__(self):
        super().__init__()
        self.nd_tensors = {}  # Dictionary to collect all ND tensors
    
    def handle_nd_tensor(self, key, data):
        # Collect all ND tensors in a dictionary
        self.nd_tensors[key] = torch.from_numpy(data)
        tqdm.write(f"5D key found in state dict! Will be saved for manual fix - {key} {data.shape}")
        
    def save_nd_tensors(self, path=None):
        if not self.nd_tensors:
            return
            
        if path is None:
            path = f"./fix_5d_tensors_{self.arch}.safetensors"
            
        tqdm.write(f"Saving {len(self.nd_tensors)} 5D tensors to {path}")
        save_file(self.nd_tensors, path)
        return path
...
class ModelWan(ModelHyVid):
    ...
    keys_ignore = [
        "scaled_fp8"  # FP8 metadata artifact, causes quantizer errors
    ]

class ModelWanRefLayers(ModelTemplate):
    arch = "wan"
    keys_detect = [
        (
            "blocks.0.self_attn.ref_adapter.to_k_ref.weight",
            "blocks.0.self_attn.ref_adapter.to_v_ref.weight",
        )
    ]

class ModelWanIPLayers(ModelTemplate):
    arch = "wan"
    keys_detect = [
        (
            "blocks.0.cross_attn.ip_adapter.to_k_ip.weight",
            "blocks.0.cross_attn.ip_adapter.to_v_ip.weight",
        )
    ]

class ModelWanAudio(ModelTemplate):
    arch = "wan"
    keys_detect = [
        (
            "audio_model.blocks.0.cross_attn.k.weight",
            "audio_model.blocks.0.cross_attn.k_fusion.weight",
            "audio_model.blocks.0.modulation",
        )
    ]
    keys_hiprec = [
        ".modulation"  # nn.parameter, keep in FP32 like main Wan model
    ]
...
arch_list = [ModelFlux, ModelSD3, ModelAura, ModelHiDream, CosmosPredict2, ModelLTXV, ModelHyVid, ModelWan, ModelWanRefLayers, ModelWanIPLayers, ModelWanAudio, ModelSDXL, ModelSD1, ModelLumina2]
...
def handle_tensors(writer, state_dict, model_arch):
    ...
    for key, data in tqdm(state_dict.items()):
        ...
        # Skip scalar tensors (0D) - they cause quantizer errors
        if len(data.shape) == 0:
            tqdm.write(f"Filtering scalar tensor: '{key}'")
            continue

        # Check if key contains any special strings that should be handled separately
        if any(special_str in key for special_str in model_arch.keys_special):
            if hasattr(model_arch, 'handle_nd_tensor'):
                # Handle dtype conversion before numpy conversion for special keys
                if data.dtype == torch.bfloat16:
                    data_converted = data.to(torch.float32)
                elif data.dtype in [getattr(torch, "float8_e4m3fn", "_invalid"), getattr(torch, "float8_e5m2", "_invalid")]:
                    data_converted = data.to(torch.float16)
                else:
                    data_converted = data
                
                data_numpy = data_converted.numpy() if hasattr(data_converted, 'numpy') else data_converted.detach().numpy()
                model_arch.handle_nd_tensor(key, data_numpy)
                continue
            else:
                tqdm.write(f"Special key detected but no handler available: '{key}'")
        ...
        if old_dtype in (torch.float32, torch.bfloat16):
            if n_dims == 1:
                ...
                print(f"High precision tensor detected: {key}")
...
def convert_file(path, dst_path=None, interact=True, overwrite=False):
    ...
    # Save any collected ND tensors
    fix_path = None
    if hasattr(model_arch, 'save_nd_tensors'):
        fix_path = model_arch.save_nd_tensors()
    
    if fix_path and os.path.isfile(fix_path):
        logging.warning(f"\n### Warning! Fix file created at '{fix_path}'")
        logging.warning(f" Found {len(model_arch.nd_tensors)} tensors with more than {MAX_TENSOR_DIMS} dimensions")
        logging.warning(" You need to run 'fix_5d_tensors.py' after quantization.")
